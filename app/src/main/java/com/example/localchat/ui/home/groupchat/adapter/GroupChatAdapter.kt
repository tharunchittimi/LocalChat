package com.example.localchat.ui.home.groupchat.adapter
...
class GroupChatAdapter(val currentLoggedInUserData: UserEntity?) :
        ...
        return when (viewType) {
            OTHERS_CHAT -> {
                val view = LayoutInflater.from(parent.context)
                    .inflate(R.layout.inflate_group_receive_chat_item, parent, false)
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        when (holder) {
            is ReceiveViewHolder -> {
                (arrayList[holder.adapterPosition] as? RoomMessageRelation)?.let {
                    var userNameTextColor = it.colorCode
                    if (it.colorCode == null) {
                        userNameTextColor = getUserDefinedColor(it)
                        it.colorCode = userNameTextColor
                    }
                    holder.itemView.tvReceiveDataUserName?.setTextColor(
                        userNameTextColor ?: R.color.white
                    )
                    holder.itemView.tvReceiveDataUserName?.text = it.userName
    ...
    private fun getUserDefinedColor(roomMessageRelation: RoomMessageRelation): Int? {
        return if (colorHashMap.containsKey(roomMessageRelation.fromUserId)) {
            colorHashMap[roomMessageRelation.fromUserId]
        } else {
            val textColor = Utils.getRandomHSVColor()
            colorHashMap[roomMessageRelation.fromUserId ?: ""] = textColor
            textColor
        }
    }
    ...
    fun addItems(list: ArrayList<RoomMessageRelation>) {
        clearList()
    ...
    fun addSendMessage(sendModelData: RoomMessageRelation) {
    ...
    private fun addHeaders(list: ArrayList<RoomMessageRelation>): ArrayList<BaseWithHeader> {
